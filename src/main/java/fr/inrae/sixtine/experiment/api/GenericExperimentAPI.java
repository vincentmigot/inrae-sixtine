...
import fr.inrae.sixtine.experiment.dal.ExperimentDAO;
 ...
 * @author Renaud COLIN
 */
@Api("GenericExperiments")
@Path("/sixtine/experiment")
@ApiCredentialGroup(
        groupId = GenericExperimentAPI.CREDENTIAL_EXPERIMENT_GROUP_ID,
        groupLabelKey = GenericExperimentAPI.CREDENTIAL_EXPERIMENT_GROUP_LABEL_KEY
)
public class GenericExperimentAPI {
    ...
    @GET
    @Path("get/{uri}/detail")
    @ApiOperation("Get experiment details by URI")
    @ApiProtected
    @ApiCredential(
            credentialId = CREDENTIAL_EXPERIMENT_READ_ID,
            credentialLabelKey = CREDENTIAL_EXPERIMENT_READ_LABEL_KEY
    ...
    @ApiResponses(value = {
        @ApiResponse(code = 200, message = "Experiment retrieved", response = PropertiesMapDTO.class),
        @ApiResponse(code = 500, message = "Internal Server Error", response = ErrorResponse.class)})
    public Response getExperimentDetails(
        ...
        try {
            ...
            if (model != null) {
                PropertiesMapDTO result = new PropertiesMapDTO();
                Map<String, List<PropertyDTO>> resulMap = new HashMap<>();

                model.getRelations().forEach((relation) -> {
                    String relationURI = relation.getProperty().getURI();
                    PropertyDTO pDTO = new PropertyDTO();
                    try {
                        pDTO.setRdfProperty(new URI(relationURI));
                    } catch (URISyntaxException ex) {
                        // should not happned
                    }
                    pDTO.setValue(relation.getValue());
                    if (!resulMap.containsKey(relationURI)) {
                        resulMap.put(relationURI, new ArrayList<>());
                    }
                    resulMap.get(relationURI).add(pDTO);
                });
                result.setProperties(resulMap);
